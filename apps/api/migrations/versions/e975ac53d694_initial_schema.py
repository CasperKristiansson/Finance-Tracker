...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "account_type",
            sa.Enum("NORMAL", "DEBT", "INVESTMENT", name="accounttype"),
            nullable=False,
        ),
    ...
    op.create_table(
        ...
        sa.Column(
            "category_type",
            sa.Enum("INCOME", "EXPENSE", "ADJUSTMENT", "LOAN", "INTEREST", name="categorytype"),
            nullable=False,
        ),
    ...
    op.create_table(
        ...
        sa.Column(
            "code",
            sa.Enum(
                "RETAINED_EARNINGS", "INTEREST_EXPENSE", "UNASSIGNED", name="systemaccountcode"
            ),
            nullable=False,
        ),
    ...
    op.create_table(
        ...
        sa.Column(
            "period", sa.Enum("MONTHLY", "QUARTERLY", "YEARLY", name="budgetperiod"), nullable=False
        ),
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["batch_id"], ["transaction_import_batches.id"], ondelete="CASCADE"
        ),
        sa.PrimaryKeyConstraint("id"),
    )
    op.create_table(
        ...
        sa.PrimaryKeyConstraint("id"),
        sa.UniqueConstraint(
            "snapshot_id", "name", "currency", name="uq_investment_holding_snapshot_name_currency"
        ),
    )
    op.create_table(
        "loans",
        ...
        sa.Column(
            "interest_compound",
            sa.Enum("DAILY", "MONTHLY", "YEARLY", name="interestcompound"),
            nullable=False,
        ),
    ...
    op.create_table(
        ...
        sa.Column(
            "suggested_subscription_confidence", sa.Numeric(precision=5, scale=2), nullable=True
        ),
    ...
    op.create_table(
        ...
        sa.Column(
            "created_source",
            sa.Enum("MANUAL", "IMPORT", "SYSTEM", name="createdsource"),
            nullable=False,
        ),
        sa.Column("created_at", sa.DateTime(), nullable=False),
        sa.Column("updated_at", sa.DateTime(), nullable=False),
        ...
        sa.Column(
            "status",
            sa.Enum("RECORDED", "IMPORTED", "REVIEWED", "FLAGGED", name="transactionstatus"),
            nullable=False,
        ),
        sa.ForeignKeyConstraint(["category_id"], ["categories.id"], ondelete="SET NULL"),
        sa.ForeignKeyConstraint(
            ["import_batch_id"], ["transaction_import_batches.id"], ondelete="SET NULL"
        ),
        sa.ForeignKeyConstraint(["subscription_id"], ["subscriptions.id"], ondelete="SET NULL"),
        sa.PrimaryKeyConstraint("id"),
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["ledger_transaction_id"], ["transactions.id"], ondelete="SET NULL"
        ),
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(["transaction_id"], ["transactions.id"], ondelete="CASCADE"),
        sa.ForeignKeyConstraint(
            ["transaction_leg_id"], ["transaction_legs.id"], ondelete="SET NULL"
        ),
        sa.PrimaryKeyConstraint("id"),
    )
