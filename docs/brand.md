...
- Primary UI primitives: shadcn/ui + Radix UI. When designing/implementing, first check if an existing local component or shadcn/Radix pattern exists for the pattern. If unsure, consult official package documentation.
...
- Always check existing local components and shadcn/Radix patterns before creating custom components; use official package documentation for props and accessibility details when needed.
